...
func TestSharedMailbox(t *testing.T) {
	...
	mb, err := CreateSharedMailbox(id, id, id, []string{"test"}, []string{"test"}, []string{"test"})
	if err != nil {
	...
	mb2, err := GetSharedMailbox(mb.ExchangeObjectId)
	if err != nil {
	...
	newName := "New Name"
	_, err = UpdateSharedMailbox(mb.ExchangeObjectId, newName)
	if err != nil {
	...
	err = DeleteSharedMailbox(mb.ExchangeObjectId)
	if err != nil {
...
func TestMemberships(t *testing.T) {
	...
	t.Log("Creating Shared Mailbox")
	id := fmt.Sprintf("test-%s", uuid.New())
	mb, err := CreateSharedMailbox(id, id, id, []string{}, []string{}, []string{})
	if err != nil {
		t.Fatalf("Failed to create shared mailbox: %s", err)
	}
	...
	mb2, err := AddSharedMailboxMembers(mb.ExchangeObjectId, []string{"alexw"})
	if err != nil {
	...
	mb2, err = AddSharedMailboxMembers(mb.ExchangeObjectId, []string{"ChristieC"})
	if err != nil {
		t.Fatalf("Failed to add members: %s", err)
	}
	...
	mb2, err = RemoveSharedMailboxMembers(mb.ExchangeObjectId, []string{"alexw"})
	if err != nil {
	...
	mb2, err = AddSharedMailboxReaders(mb.ExchangeObjectId, []string{"alexw"})
	if err != nil {
	...
	mb2, err = AddSharedMailboxReaders(mb.ExchangeObjectId, []string{"ChristieC"})
	if err != nil {
		t.Fatalf("Failed to add readers: %s", err)
	}
	...
	mb2, err = RemoveSharedMailboxReaders(mb.ExchangeObjectId, []string{"alexw"})
	if err != nil {
